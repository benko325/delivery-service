...
export async function up(db: Kysely<unknown>): Promise<void> {
    ...
    await db.schema
        ...
        .addColumn('opening_hours', 'jsonb', (col) => col.notNull().defaultTo(sql`'{}'::jsonb`))
    ...
    await db.schema
        ...
        .addColumn('restaurant_id', 'uuid', (col) =>
            col.notNull().references('restaurants.restaurants.id').onDelete('cascade'),
        )
        .addColumn('name', 'varchar(255)', (col) => col.notNull())
        .addColumn('description', 'text', (col) => col.notNull())
        .addColumn('price', 'decimal(10, 2)', (col) => col.notNull())
        .addColumn('currency', 'varchar(3)', (col) => col.notNull().defaultTo('USD'))
        .addColumn('category', sql`restaurants.menu_item_category`, (col) => col.notNull())
        .addColumn('image_url', 'varchar(500)')
        .addColumn('is_available', 'boolean', (col) => col.notNull().defaultTo(true))
        .addColumn('preparation_time', 'integer', (col) => col.notNull().defaultTo(15))
        .addColumn('created_at', 'timestamptz', (col) => col.notNull().defaultTo(sql`now()`))
        .addColumn('updated_at', 'timestamptz', (col) => col.notNull().defaultTo(sql`now()`))
        .execute();
